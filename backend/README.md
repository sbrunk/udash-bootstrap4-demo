...
* `eu.iums.bootstrap4.backend.css` - the code rendering the styles defined in the `shared` module.
* `eu.iums.bootstrap4.backend.rpc` - an implementation of the server RPC interfaces.
* `eu.iums.bootstrap4.backend.server` - the code setting up [Jetty](https://www.eclipse.org/jetty/) with required servlets.
* `eu.iums.bootstrap4.backend.services` - services encapsulating the main business logic of the application.
* `eu.iums.bootstrap4.backend.spring` - utilities handling [Spring](https://spring.io/) dependency injection.
